...
public class LoomoRecognitionService {
    ...
    public void sendSpeechToText() {
        new AsyncTask<String, Void, Void>() {

            @Override
            protected Void doInBackground(String... voids) {
                InputStream googleCredentialsStream = null;
                try {
                    googleCredentialsStream = context.getAssets().open("API-key.json");
                    CredentialsProvider credentialsProvider = FixedCredentialsProvider.create(ServiceAccountCredentials.fromStream(googleCredentialsStream));
                ...
                File file = new File(context.getExternalCacheDir().getAbsolutePath() + "/test.3gpp");
